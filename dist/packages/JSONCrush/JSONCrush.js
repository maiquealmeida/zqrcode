...
var _default = exports.default = {
  crush: (string, maxSubstringLength = 50) => {
    ...
    const JSCrush = (string, replaceCharacters) => {
      ...
      let substringCount = {};
      for (let substringLength = 2; substringLength < maxSubstringLength; substringLength++) for (let i = 0; i < string.length - substringLength; ++i) {
      ...
      // loop while string can be crushed more
      {
        ...
        for (let substring in substringCount) {
          ...
          if (lengthDelta <= 0) delete substringCount[substring];else if (lengthDelta > bestLengthDelta) {
...
const JSONCrushSwap = (string, forward = 1) => {
  ...
  if (forward) for (let i = 0; i < swapGroups.length; ++i) string = swapInternal(string, swapGroups[i]);else for (let i = swapGroups.length; i--;) string = swapInternal(string, swapGroups[i]);
  return string;
};